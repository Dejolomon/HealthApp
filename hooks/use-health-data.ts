...
export function useHealthData() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const nextToday = { ...defaultDay(), ...(profileData.weight && profileData.bmi ? { weight: profileData.weight, bmi: profileData.bmi } : {}) };
  ...
  const updateGoals = useCallback(
    async (patch: Partial<DailyGoals>) => {
      setGoals((prev) => {
  ...
  const setMetric = useCallback(
    <K extends keyof DaySummary>(key: K, value: DaySummary[K]) => updateToday({ [key]: value } as Partial<DaySummary>),
    [updateToday]
  );
  ...
  const recommendations = useMemo(() => {
    const recs: string[] = [];
    if (state.today.steps < goals.steps) recs.push('Time for a 10-minute walk to boost your steps.');
    if (state.today.water < goals.water) recs.push('Drink 2 glasses of water to stay hydrated.');
    if (state.today.sleep < goals.sleep) recs.push('Aim for at least 7 hours of sleep tonight.');
    if (state.today.activity < 60) recs.push('Add a short stretching or cycling session.');
    if (state.today.bloodSugar > 110) recs.push('Take a short walk to help stabilize blood sugar.');
    if (state.today.calories > goals.calories) recs.push('Balance calories with a light, protein-rich snack.');
    if (recs.length === 0) recs.push('Great job! Keep maintaining today’s healthy rhythm.');
